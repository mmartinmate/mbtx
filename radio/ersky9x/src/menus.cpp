...
"\005\002V2x2\0V2x2\0JXD506\0"
"\006\005DSM\0DSM2-22\0DSM2-11\0DSMX-22\0DSMX-11\0AUTO\0"
"\007\0Devo\0"
"\008\005YD717\0YD717\0SKYWLKR\0SYMAX4\0XINXUN\0NIHUI\0"
"\009\002KN\0WLTOYS\0FEILUN\0"
"\010\002SymaX\0SYMAX\0SYMAX5C\0"
"\011\005SLT\0SLT_V1\0SLT_V2\0Q100\0Q200\0MR100\0"
"\012\007CX10\0GREEN\0BLUE\0DM007\0---\0J3015_1\0J3015_2\0MK33041\0"
"\013\002CG023\0CG023\0YD829\0"
"\014\004Bayang\0Bayang\0H8S3D\0X16_AH\0IRDRONE\0"
"\015\004FrskyX\0CH_16\0CH_8\0EU_16\0EU_8\0"
"\016\0ESky\0"
"\017\005MT99xx\0MT\0H7\0YZ\0LS\0FY805\0"
"\018\006MJXq\0WLH08\0X600\0X800\0H26D\0E010\0H26WH\0"
"\019\0Shenqi\0"
"\020\002FY326\0FY326\0FY319\0"
"\021\0SFHSS\0"
"\022\0J6PRO\0"
"\023\0FQ777\0"
"\024\0ASSAN\0"
"\025\0FrskyV\0"
"\026\004HONTAI\0HONTAI\0JJRCX1\0X5C1\0FQ777_951\0"
"\027\0OpnLrs\0"
"\028\004AFHDS2A\0PWM_IBUS\0PPM_IBUS\0PWM_SBUS\0PPM_SBUS\0"
"\029\003Q2X2\0Q222\0Q242\0Q282\0"
"\030\006WK2x01\0WK2801\0WK2401\0W6_5_1\0W6_6_1\0W6_HEL\0W6_HEL_I\0"
"\031\004Q303\0Q303\0CX35\0CX10D\0CX10WD\0"
"\032\0GW008\0"
"\033\0DM002\0"
"\034\010CABELL\0CAB_V3\0C_TELEM\0-\0-\0-\0-\0F_SAFE\0UNBIND\0"
"\035\0ESKY150\0"
"\036\004H8_3D\0H8_3D\0H20H\0H20Mini\0H30Mini\0"
"\037\003CORONA\0COR_V1\0COR_V2\0FD_V3\0"
"\038\0CFlie\0"
"\039\003Hitec\0OPT_FW\0OPT_HUB\0Minima\0"
"\040\0WFLY\0"
"\041\0BUGS\0"
"\042\002BUGSMINI\0BUGSMINI\0BUGS3H\0"
"\043\001Traxxas\0RX6519\0"
"\044\0NCC1701\0"
"\045\003E01X\0E012\0E015\0E016H\0"
"\046\0V911S\0"
"\047\002GD00X\0GD_V1\0GD_V2\0"
"\048\0V761\0"
"\049\0KF606\0"
"\050\02Redpine\0Fast\0Slow\"
"\051\001Potensic\A20\0"
"\052\001ZSX\0280\0"
"\053\001Flyzone\0FZ-410\0"
"\054\0Scanner\0"
"\055\002FrskyX_RX\0FCC\0EU_LBT\0"
"\063\002XN_DUMP\0250K\01M\02M\0"	
"\0" ;
...
void alphaEditName( uint8_t x, uint8_t y, uint8_t *name, uint8_t len, uint8_t type, uint8_t *heading )
{
	...
	if ( type & ~ALPHA_NO_NAME )
	{
		...
		{
			SharedMemory.Alpha.AlphaLength = len ;
			SharedMemory.Alpha.PalphaText = name ;
			SharedMemory.Alpha.PalphaHeading = heading ;
			s_editMode = 0 ;
    	killEvents(Tevent) ;
			Tevent = 0 ;
...
//#endif
  ...
  index = s_pgOfs ;
	int8_t x = sub - index ;
  if(sub<1) index = 0 ;
  else if(x>5) index = sub - 5 ;
  else if(x<0) index = sub ;
	s_pgOfs = index ;

	index *= width ;
	for ( i = 0 ; i < width * 6 ; i += 1 )
	{
		uint8_t x = (i%4)* 31 ;
		if ( width == 2 )
		{
...
//			}
	...
	if ( TextFound )
	{
		lcd_putsAtt( 0, 7*FH, (char *)&SharedMemory.TextControl.TextMenuBuffer[TextFound], 0 ) ;
	}
//	else
//	{
...
//  uint8_t k = 0;
  ...
  t_pgOfs = evalOffset(sub);
	...
	if ( event == EVT_ENTRY )
	{
		...
		SharedMemory.Alpha.AlphaChanged = 0 ;
	}
  ...
  lcd_outdez( 18*FW, 0, sub+1 ) ;
	...
	{
		SharedMemory.Alpha.AlphaLength = 21 ;
		SharedMemory.Alpha.PalphaText = &SharedMemory.TextControl.TextMenuBuffer[sub*21] ;
		SharedMemory.Alpha.PalphaHeading = (uint8_t *)"Note text" ;
		s_editMode = 0 ;
    killEvents(event) ;
		pushMenu(menuProcAlpha) ;
		SharedMemory.Alpha.AlphaChanged = 1 ;
	}
	if ( (event == EVT_KEY_BREAK(KEY_EXIT) ) || (event == EVT_KEY_LONG(KEY_EXIT) ) )
	{
		if ( SharedMemory.Alpha.AlphaChanged )
		{
			writeModelNotes() ;		
		}
	}
...
//				svalue = editMultiProtocol( svalue, ( sub == subN ) ) ;
				...
				{
					attr = blink ;
			 		CHECK_INCDEC_H_MODELVAR_0( svalue, 63 ) ;
				}
  ...
  if(sub==subN)
	{
		...
		MaskRotaryLong = 1 ;
		if ( ( value > 0) && ( checkForExitEncoderLong( event ) ) )
		{
			SingleExpoChan = 1 ;
			s_expoChan = value-1 ;
			s_editMode = 0 ;
...
//extern uint16_t XjtHbeatOffset ;
//  lcd_outhex4( 0,  4*FH, XjtHbeatOffset ) ;
...
void menuProcAlpha(uint8_t event)
{
	struct t_alpha *Palpha = &SharedMemory.Alpha ;
	char *keytext ;
...
#if defined(PCBSKY) || defined(PCBX9D) || defined(PCB9XT) || defined(PCBX12D) || defined(PCBX10)
 #ifndef REVA
		disp_datetime( 1*FH ) ;
 #endif
...
extern uint16_t Image_height ;
	...
	extern void lcd_picture( uint16_t i_x, uint16_t i_y ) ;
			lcd_picture( 480-128, 128+64 ) ;
		}
		else
		{
			lcdDrawSolidFilledRectDMA( 480-128, 128+64, 128, 64, 0 ) ;
			pushPlotType( PLOT_WHITE ) ;
			lcd_vline( 240-64, 64+32, 32 ) ;
			lcd_vline( 239, 64+32, 32 ) ;
			lcd_hline( 240-64, 64+32, 64 ) ;
			lcd_hline( 240-64, 95+32, 64 ) ;
			popPlotType() ;
			lcd_putsAtt( 240-64+26, 68+32, "No", INVERS ) ;
			lcd_putsAtt( 240-64+26-9, 76+32, "Image", INVERS ) ;
		}
		PictureDrawn += 1 ;
	}
...
// Mavlink - home direction and distance on graphic?								
						 ...
						 else
						 {
							...
							lcd_puts_P( 11*FW-1, 0*FH, XPSTR("tssi ")) ;	// Space to blank battery voltage
...
extern uint16_t Image_height ;
		...
		{
	extern void lcd_picture( uint16_t i_x, uint16_t i_y ) ;
			lcd_picture( (480-Image_width)/2, 128 - Image_height / 2 ) ;
		}
		else
		{
...
static void displayIndex( const uint16_t *strings, uint8_t extra, uint8_t lines, uint8_t highlight )
{
	...
	if ( extra == 8 )
	{
		...
		lcd_puts_P( 69 + X12OFFSET, 2*FH, PSTR(strings[7]) ) ;
	}
	for ( uint8_t i = 0 ; i < lines ; i += 1 )
	{
		lcd_puts_P( 1 + X12OFFSET, (i+3)*FH, PSTR(strings[i]) ) ;
		if ( i < extra )
		{
			lcd_puts_P( 69 + X12OFFSET, (i+3)*FH, PSTR(strings[i+offset]) ) ;
		}
	} 
	...
	pushPlotType( PLOT_BLACK ) ;
#endif
	lcd_vline( 67 + X12OFFSET, 2*FH, 63 ) ;
#if defined(PCBX12D) || defined(PCBX10)
		popPlotType() ;
#endif
	...
	if ( highlight )
	{
		if ( highlight > 7 )
		{
			lcd_char_inverse( 68 + X12OFFSET, (highlight-offset+2)*FH, 60, 0 ) ;
		}
		else
		{
			lcd_char_inverse( 0 + X12OFFSET, (highlight+2)*FH, 67, 0 ) ;
		}
	}
}
#else
...
void menuProcBt(uint8_t event)
{
  MENU(XPSTR("BT"), menuTabStat, e_bluetooth, 7, {0/*, 0*/});
	...
	uint16_t *p = (uint16_t *)&BtRx_fifo ;
#endif
#if defined(PCBX7) || defined(PCBX9D)
	uint16_t *p = (uint16_t *)&BtRx_fifo ;
#endif
	uint32_t i ;
	uint16_t x ;
...
#define STATUS_VERTICAL		256

extern uint8_t speaker[] ;
void displayStatusLine( uint32_t scriptPercent )
{
	lcdDrawSolidFilledRectDMA( 0, STATUS_VERTICAL, 480, 16, LCD_STATUS_GREY ) ;
#ifdef WHERE_TRACK
	notePosition('P') ;
#endif
	putsTime( 224, (STATUS_VERTICAL/2)+1, Time.hour*60+Time.minute, 0, 0, LCD_BLACK, LCD_STATUS_GREY ) ;
#ifdef WHERE_TRACK
	notePosition('R') ;
#endif
...
extern uint8_t CurrentVolume ;
	pushPlotType( PLOT_BLACK ) ;
	lcd_hbar( 185, (STATUS_VERTICAL/2)+1, 23, 6, (CurrentVolume*100+16)/23 ) ;
	popPlotType() ;
...
extern uint8_t LogsRunning ;
	if ( LogsRunning & 1 )
	{
		if ( BLINK_ON_PHASE )
		{
			lcd_img( 165, (STATUS_VERTICAL/2)+1, IconX12Logging, 0, 0, LCD_BLACK, LCD_STATUS_GREY ) ;
		}
	}
extern uint32_t IdlePercent ;
  lcd_outdezNAtt( 4*FW, (STATUS_VERTICAL/2)+1, IdlePercent ,PREC2, 4, LCD_BLACK, LCD_STATUS_GREY ) ;
	if ( scriptPercent )
	{
extern uint32_t BasicExecTime ;
	  lcd_outdezNAtt( 8*FW, (STATUS_VERTICAL/2)+1, BasicExecTime ,PREC1, 4, LCD_BLACK, LCD_STATUS_GREY ) ;
	}
}
#endif
